...
@class JDDOF_GCDAsyncReadPacket;
@class JDDOF_GCDAsyncWritePacket;
@class JDDOF_GCDAsyncSocketPreBuffer;
...
extern NSString *const JDDOF_GCDAsyncSocketException;
extern NSString *const JDDOF_GCDAsyncSocketErrorDomain;

extern NSString *const JDDOF_GCDAsyncSocketQueueName;
extern NSString *const JDDOF_GCDAsyncSocketThreadName;

#if SECURE_TRANSPORT_MAYBE_AVAILABLE
extern NSString *const JDDOF_GCDAsyncSocketSSLCipherSuites;
#if TARGET_OS_IPHONE
extern NSString *const JDDOF_GCDAsyncSocketSSLProtocolVersionMin;
extern NSString *const JDDOF_GCDAsyncSocketSSLProtocolVersionMax;
#else
extern NSString *const JDDOF_GCDAsyncSocketSSLDiffieHellmanParameters;
#endif
#endif

enum JDDOF_GCDAsyncSocketError
{
	JDDOF_GCDAsyncSocketNoError = 0,           // Never used
	JDDOF_GCDAsyncSocketBadConfigError,        // Invalid configuration
	JDDOF_GCDAsyncSocketBadParamError,         // Invalid parameter was passed
	JDDOF_GCDAsyncSocketConnectTimeoutError,   // A connect operation timed out
	JDDOF_GCDAsyncSocketReadTimeoutError,      // A read operation timed out
	JDDOF_GCDAsyncSocketWriteTimeoutError,     // A write operation timed out
	JDDOF_GCDAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	JDDOF_GCDAsyncSocketClosedError,           // The remote peer closed the connection
	JDDOF_GCDAsyncSocketOtherError,            // Description provided in userInfo
};
typedef enum JDDOF_GCDAsyncSocketError JDDOF_GCDAsyncSocketError;

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark -
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

@interface JDDOF_GCDAsyncSocket : NSObject
...
@protocol JDDOF_GCDAsyncSocketDelegate
@optional
...
**/
- (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(JDDOF_GCDAsyncSocket *)sock;
...
**/
- (void)socket:(JDDOF_GCDAsyncSocket *)sock didAcceptNewSocket:(JDDOF_GCDAsyncSocket *)newSocket;

/**
 * Called when a socket connects and is ready for reading and writing.
 * The host parameter will be an IP address, not a DNS name.
**/
- (void)socket:(JDDOF_GCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port;

/**
 * Called when a socket has completed reading the requested data into memory.
 * Not called if there is an error.
**/
- (void)socket:(JDDOF_GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)socket:(JDDOF_GCDAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(JDDOF_GCDAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)socket:(JDDOF_GCDAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(JDDOF_GCDAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(JDDOF_GCDAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(JDDOF_GCDAsyncSocket *)sock;
...
**/
- (void)socketDidDisconnect:(JDDOF_GCDAsyncSocket *)sock withError:(NSError *)err;
...
**/
- (void)socketDidSecure:(JDDOF_GCDAsyncSocket *)sock;
